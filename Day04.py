  ...
  def __init__(self, inputStr):
    ...
    for line in inputStr.splitlines():
      for pair in line.split():
        ...
        self.fields[splitPair[0]] = splitPair[1]
    
  def is_valid(self, use_old_test = True):
    if use_old_test:
...
passports = [ Passport(passportStr) for passportStr in test.split('\n\n')]
valid_passports = [ passport for passport in passports if passport.is_valid() ]
# Part 1, number of valid passports (allowing cid to be optional)
print(len(valid_passports))
# Part 2, number of valid passports but now each field has its own checks
...
passports = [ Passport(passportStr) for passportStr in input.split('\n\n')]
valid_passports = [ passport for passport in passports if passport.is_valid(False) ]
invalid_passports = [ passport for passport in passports if passport.is_valid(False) == False ]
print(len(valid_passports), len(invalid_passports))