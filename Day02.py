...
class Policy:
  def __init__(self, policy_str):
    split_policy = policy_str.split()
    self.Min, self.Max = split_policy[0].split('-')
    self.Min = int(self.Min)
    self.Max = int(self.Max)
    #self.Max = split_policy[0].split('-')[1]
    self.Val = split_policy[1]
  
  def __repr__(self):
    return "<Policy Val:%s Min:%i Max:%i>" % (self.Val, self.Min, self.Max)

class Password:
  def __init__(self, input_str):
    split_input = input_str.split(':')
    self.Policy = Policy(split_input[0])
    self.Pass = split_input[1].strip()
  
  def is_valid(self, use_old_policy = True):
    if use_old_policy:
      num_val = self.Pass.count(self.Policy.Val)
      if self.Policy.Min <= num_val and num_val <= self.Policy.Max:
        return True
    else:
      min_val = self.Pass[self.Policy.Min-1]
      max_val = self.Pass[self.Policy.Max-1]
      if min_val == self.Policy.Val and max_val == self.Policy.Val:
        return False
      elif min_val == self.Policy.Val or max_val == self.Policy.Val:
        return True
    return False
  
  def __repr__(self):
    return "<Password Pass:%s Policy:%s>" % (self.Pass, self.Policy)

pass_list = [Password(line) for line in input.splitlines()]

# Part 1, find number of valid passwords
print(len([p for p in pass_list if p.is_valid()]))
# Part 2, same thing but use "new" policy (min/max are actually positions the value should appear at)
print(len([p for p in pass_list if p.is_valid(False)]))